...
# python crawl.py -q myongji_univ -n 4
...
def extractImgUrl(data):
    soup = BeautifulSoup(data)
    result = soup.select("div.KL4Bh > img")[0]['srcset']
    return result
...
def runCrawl(limitNum = 0, queryList = [], is_all_comments=False):
    ...
    time.sleep(2.0)
    browser.driver.find_element_by_name("username").send_keys("leet41627@gmail.com")
    time.sleep(2.0)
    browser.driver.find_element_by_name("password").send_keys("bluesun")
    time.sleep(2.0)
